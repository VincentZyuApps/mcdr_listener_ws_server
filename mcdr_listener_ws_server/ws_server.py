...
class WebSocketHandler:
    def __init__(
        ...
        self.enable_remote_exec_command = enable_remote_exec_command
    ...
    def _verify_token(self, websocket) -> bool:
        ...
        try:
            ...
            return len(token_list) > 0 and token_list[0] == self.ws_token
        except Exception:
            return False
    ...
    async def handle_message(self, message: str, websocket):
        try:
            ...
            if msg_type == "command":
                ...
                group_name = sanitize_for_console_encoding(
                    data.get("group_name", f"群{group_id}")
                )
    ...
    async def _handle_command(self, data: dict, websocket):
        ...
        if not self.enable_remote_exec_command:
            await self.safe_send(
                websocket,
                {
                    ...
                    "command": command,
                    "ok": False,
                    "error": str(tr(self.server, "remote_command.disabled")),
                },
            )
            return

        if self.remote_exec_command_whitelist:
            allowed = any(
                command.startswith(prefix)
                for prefix in self.remote_exec_command_whitelist
            )
            if not allowed:
                await self.safe_send(
                    websocket,
                    {
                        ...
                        "error": str(
                            tr(
                                self.server,
                                "remote_command.not_in_whitelist",
                                command=command,
                            )
                        ),
                    },
        ...
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(None, self.server.rcon_query, command),
                timeout=self.remote_exec_command_timeout_sec,
            )
            ...
            if result is None:
                ...
                await self.safe_send(
                    websocket,
                    {
                        ...
                        "ok": False,
                        "error": str(
                            tr(self.server, "remote_command.rcon_returned_none")
                        ),
                    },
                )
                return

            if len(result) > self.remote_exec_result_max_length:
                result = (
                    result[: self.remote_exec_result_max_length] + "\n...(truncated)"
                )
            ...
            await self.safe_send(
                websocket,
                {
                    ...
                    "error": str(
                        tr(
                            self.server,
                            "remote_command.timeout",
                            timeout_sec=self.remote_exec_command_timeout_sec,
                        )
                    ),
                },
            )
    ...
    def broadcast_message_with_images(
        self, group_id: str, group_name: str, nickname: str, message: str, images: list
    ):
        """
        广播包含图片的消息给所有在线玩家
        """
        # 获取所有在线玩家
        player_list = (
            self.server.get_plugin_list()
            if hasattr(self.server, "get_plugin_list")
            else []
        )
        ...
        def sanitize_text(text: str) -> str:
            ...
            text = text.replace("§", "")
            text = "".join(
                c if (ord(c) >= 0x20 and ord(c) != 0x7F) else " " for c in text
            )
            import re
        ...
        try:
            ...
            if self.image_handler and "<img:" in message:
                # 处理图片标记（直接传递图片列表）
                processed_msg = self.image_handler.replace_image_markers(
                    message, images
                )
                prefix_json = json.dumps(
                    prefix_components, ensure_ascii=False, separators=(",", ":")
                )
                prefix_json_inner = prefix_json[1:-1]
                full_message = (
                    f'[""'
                    + (f",{prefix_json_inner}," if prefix_json_inner else ",")
                    + f"{processed_msg}]"
                )
