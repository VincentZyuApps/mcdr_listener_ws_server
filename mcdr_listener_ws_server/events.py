...
def on_info(server: PluginServerInterface, info: Info) -> None:
    if not info.is_user and re.fullmatch(
        r"Starting Minecraft server on \S*", info.content
    ):
        server.logger.info(
            "【-- Server starting --】 {}".format(info.content.rsplit(" ", 1)[1])
        )
...
def on_user_info(server: PluginServerInterface, info: Info, ws_handler) -> None:
    ...
    if ws_handler is None:
        server.logger.warning(
            "【-- WS skipped --】 player chat broadcast skipped because handler is not initialized"
        )
        return
...
def on_player_joined(
    server: PluginServerInterface, player: str, info: Info, ws_handler, player_logger
) -> None:
    ...
    if ws_handler is None:
        server.logger.warning(
            "【-- WS skipped --】 join broadcast skipped because handler is not initialized"
        )
        return
    ws_handler.broadcast(
        {
...
def on_player_left(
    server: PluginServerInterface, player: str, ws_handler, player_logger
) -> None:
    ...
    if ws_handler is None:
        server.logger.warning(
            "【-- WS skipped --】 leave broadcast skipped because handler is not initialized"
        )
        return
    ws_handler.broadcast(
        {
